...
class CAB500 : public CurrentSensor
{
public:
    CAB500(std::function<void(float)> onUpdate, uint32_t id1 = 0x3C0, uint32_t id2 = 0x3C1, uint32_t id3 = 0x3C2)
    ...
    bool onReceive(const CAN_message_t &msg) override
    {
        if (msg.id == _id1 || msg.id == _id2 || msg.id == _id3)
        {
            ...
            int32_t rawCan = (msg.buf[0] << 24) | (msg.buf[1] << 16) | (msg.buf[2] << 8) | msg.buf[3];
            int32_t milliAmps = rawCan - CAB500_OFFSET;
